...
void BatchPoolLayer<Dtype>::Backward_cpu(const vector<Blob<Dtype>*>& top,
  ...
  if (has_weight){
    ...
    for (int num_t = 0; num_t < batch_size_; ++num_t)
    {
      ...
      // average gradients;
      //weight_diff[num_t] = weight_diff[num_t] / Dtype(slice_cnt);
      // gradient cut
      LOG(INFO) << "BatchPool weight_diff[" << num_t << "]: " << weight_diff[num_t];
      if(weight_diff[num_t] > Dtype(5))
      {
        weight_diff[num_t] = Dtype(5);
      }
      if(weight_diff[num_t] < Dtype(-5))
      {
        weight_diff[num_t] = Dtype(-5);
    ...
    switch (this->layer_param_.batch_pool_param().pool()) {
      ...
      case BatchPoolParameter_PoolMethod_AVE:
        ...
        {
          caffe_copy<Dtype>(slice_cnt, top_diff, bottom_diff + num_t * slice_cnt);
          caffe_scal<Dtype>(slice_cnt, Dtype(1.0) / Dtype(batch_size_), bottom_diff + num_t * slice_cnt);
        }
        break;
      case BatchPoolParameter_PoolMethod_STOCHASTIC:
        NOT_IMPLEMENTED;
