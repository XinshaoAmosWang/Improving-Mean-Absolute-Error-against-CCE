...
#include "caffe/layers/weighted_pair_loss_layer.hpp"
#include "caffe/util/math_functions.hpp"

namespace caffe {

template <typename Dtype>
void WeightedPairLossLayer<Dtype>::LayerSetUp(
...
template <typename Dtype>
void WeightedPairLossLayer<Dtype>::Reshape(
    const vector<Blob<Dtype>*>& bottom, const vector<Blob<Dtype>*>& top) {
...
template <typename Dtype>
void WeightedPairLossLayer<Dtype>::Forward_cpu(
    const vector<Blob<Dtype>*>& bottom, const vector<Blob<Dtype>*>& top) {
...
template <typename Dtype>
void WeightedPairLossLayer<Dtype>::Backward_cpu(const vector<Blob<Dtype>*>& top,
  ...
  if (propagate_down[0]) {
    ...
    for (int i = 0; i < sample_num; i++){
    	for (int j = 0; j < sample_num; j++){
    		if(j != i){
	        pair_index++;
          hardest_index_vec.push_back(pair_index);
          sum_score += pair_score[pair_index];        
	      }
    	}
    }
...
#ifdef CPU_ONLY
STUB_GPU(WeightedPairLossLayer);
#endif

INSTANTIATE_CLASS(WeightedPairLossLayer);
REGISTER_LAYER_CLASS(WeightedPairLoss);
