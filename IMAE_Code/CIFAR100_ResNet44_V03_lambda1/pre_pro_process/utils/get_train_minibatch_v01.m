function [batch_data, batch_label] = get_train_minibatch_v01 ( id_num, image_per_id, ...
                                        batch_data, batch_label, batch_size, ...
                                        class_ids,  ImageDataCell, ...
                                        crop_padding, force_square_size, ...
                                        cropped_size, ids, select_id_indexes )
    % sample severals ids
    ...
    for i = 1 : id_num
        ...
        images_of_this_id = ImageDataCell(indexes, :);
        %
        ...
        for img = 1 : image_per_id
            batch_label(:,:,:, (i-1)*image_per_id + img ) = select_id;  % [0, class-num-1]
            batch_data(:,:,:, (i-1)*image_per_id + img ) = process_imgdataint8(sampled_images_of_this_id{img,1}, ...
                                                                                    crop_padding, force_square_size, ...
                                                                                    cropped_size, true);
        end
