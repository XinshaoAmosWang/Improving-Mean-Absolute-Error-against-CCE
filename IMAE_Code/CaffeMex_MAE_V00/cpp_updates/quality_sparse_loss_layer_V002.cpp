...
void SparseLossLayer<Dtype>::Backward_cpu(const vector<Blob<Dtype>*>& top,
  ...
  for(int i = 0; i < count_; i++){
    //no gap
    if( diff_.cpu_data()[i] + margin < margin / Dtype(10) ){
      diff_.mutable_cpu_data()[i] = Dtype(1e-2);
    }
    //very huge gap
    if( diff_.cpu_data()[i] > Dtype(bottom[0]->num() / 2 ) * margin ){
      diff_.mutable_cpu_data()[i] = Dtype(-1e-1 * diff_.cpu_data()[i]);
    }
    //large gap
    if( diff_.cpu_data()[i] > margin ){
