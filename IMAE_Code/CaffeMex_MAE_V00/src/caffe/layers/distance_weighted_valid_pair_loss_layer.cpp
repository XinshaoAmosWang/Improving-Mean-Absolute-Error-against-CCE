...
#include "caffe/layers/distance_weighted_valid_pair_loss_layer.hpp"
#include "caffe/util/math_functions.hpp"

namespace caffe {

template <typename Dtype>
void DistanceWeightedValidPairLossLayer<Dtype>::LayerSetUp(
...
template <typename Dtype>
void DistanceWeightedValidPairLossLayer<Dtype>::Reshape(
    const vector<Blob<Dtype>*>& bottom, const vector<Blob<Dtype>*>& top) {
...
template <typename Dtype>
void DistanceWeightedValidPairLossLayer<Dtype>::Forward_cpu(
    const vector<Blob<Dtype>*>& bottom, const vector<Blob<Dtype>*>& top) {
...
template <typename Dtype>
void DistanceWeightedValidPairLossLayer<Dtype>::Backward_cpu(const vector<Blob<Dtype>*>& top,
  ...
  if (propagate_down[0]) {
    ...
    //LOG(INFO) << "legacy_version: " << legacy_version;
    //
    for(int i = 0; i < pair_num; i++){
      ...
      if( static_cast<int>(pair_label.cpu_data()[i]) ){
        ...
        pair_score[i] = pair_score[i] * dist; //reweight
      }
      else{
        //dissimilar pairs
        Dtype dist = sqrt(auxiLayer->dist_sq_.cpu_data()[i]);
        Dtype dist_weight = std::max(Dtype(0), margin - dist);
...
#ifdef CPU_ONLY
STUB_GPU(DistanceWeightedValidPairLossLayer);
#endif

INSTANTIATE_CLASS(DistanceWeightedValidPairLossLayer);
REGISTER_LAYER_CLASS(DistanceWeightedValidPairLoss);
