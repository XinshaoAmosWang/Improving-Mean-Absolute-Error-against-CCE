...
#include "caffe/layers/distance_weighted_valid_pair_loss_V3_layer.hpp"
#include "caffe/util/math_functions.hpp"

namespace caffe {

template <typename Dtype>
void DistanceWeightedValidPairLossV3Layer<Dtype>::LayerSetUp(
...
template <typename Dtype>
void DistanceWeightedValidPairLossV3Layer<Dtype>::Reshape(
    const vector<Blob<Dtype>*>& bottom, const vector<Blob<Dtype>*>& top) {
...
template <typename Dtype>
void DistanceWeightedValidPairLossV3Layer<Dtype>::Forward_cpu(
    const vector<Blob<Dtype>*>& bottom, const vector<Blob<Dtype>*>& top) {
...
template <typename Dtype>
void DistanceWeightedValidPairLossV3Layer<Dtype>::Backward_cpu(const vector<Blob<Dtype>*>& top,
  ...
  if (propagate_down[0]) {
    ...
    for(int i = 0; i < pair_num; i++){
      hardest_index_vec.push_back(i);
      //distance based weighting
      if( static_cast<int>(pair_label.cpu_data()[i]) ){
        //similar pairs
        Dtype dist_weight = Dtype(1);
        if( auxiLayer->dist_sq_.cpu_data()[i] < static_cast<Dtype>(1e-4) ){
          dist_weight = Dtype(0);
        }
        pair_score[i] = pair_score[i] * dist_weight; //reweight
      }
      else{
        //dissimilar pairs
        Dtype dist = std::max(Dtype(0), Dtype(margin - sqrt(auxiLayer->dist_sq_.cpu_data()[i])) );
        
        Dtype dist_weight = Dtype(1);
        if(dist*dist  < static_cast<Dtype>(1e-4) ){
          dist_weight = Dtype(0);
        }
        
        pair_score[i] = pair_score[i] * dist_weight; // reweight
      }
      sum_score += pair_score[i];
    }
...
#ifdef CPU_ONLY
STUB_GPU(DistanceWeightedValidPairLossV3Layer);
#endif

INSTANTIATE_CLASS(DistanceWeightedValidPairLossV3Layer);
REGISTER_LAYER_CLASS(DistanceWeightedValidPairLossV3);
